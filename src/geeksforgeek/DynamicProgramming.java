...
public class DynamicProgramming {
	...
	private static int computeMinimumNumberOfDrops(int numberOfEggs,
			int numberOfFloors) {
		...
		for (int i = 1; i <= numberOfFloors; ++i) {
			...
			int minimumSoFar = Math.max(
					computeMinimumNumberOfDrops(numberOfEggs - 1, i - 1), // egg
																			// breaks
					computeMinimumNumberOfDrops(numberOfEggs, numberOfFloors
							- i)); // egg didn't break
	...
	// TODO : implement memorization version of the computeMinimumNumberOfDrops
	// here.
	...
	private static int cuttingRod(int[] prices, int n) {
		...
		for (int i = 0; i < n; ++i) {
			maxSoFar = Math.max(cuttingRod(prices, n - i - 1) + prices[i],
					maxSoFar);
		}
		return maxSoFar;
	}
	...
	private static int testCuttingRod(int[] prices, int n) {
		...
		int maxSoFar = Integer.MIN_VALUE;
		for (int i = 0; i < n; ++i) {
			maxSoFar = Math.max(maxSoFar, testCuttingRod(prices, n - i - 1)
					+ prices[i]);
		}
		return maxSoFar;
	}
	...
	private static int testCuttingRodDynamicProgramming(int[] prices, int n,
		...
		int maxSoFar = Integer.MIN_VALUE;
		for (int i = 0; i < n; ++i) {
			lookupTable[n - i - 1] = testCuttingRodDynamicProgramming(prices, n
					- i - 1, lookupTable);
	...
	public void cuttingRod() {
		// optimal substructure = max(currentPrice + takingRod, currentPrice +
		// notTakingRod)
		// = max(currentPrice+prices[nextRod], currentPrice)
		int[] arr = { 1, 5, 8, 9, 10, 17, 17, 20 };
		lookup = new int[arr.length + 1];
		System.out.println("cuttingRod : " + cuttingRod(arr, arr.length) + " "
				+ calcCount);
		calcCount = 0;
		System.out.println("cuttingRodDP : "
				+ cuttingRodDynamicProgramming(arr, arr.length) + " "
				+ calcCount);
		calcCount = 0;
		System.out.println("cuttingRod : " + testCuttingRod(arr, arr.length)
				+ " " + calcCount);
		calcCount = 0;
		System.out.println("cuttingRodDP : "
				+ testCuttingRodDynamicProgramming(arr, arr.length) + " "
				+ calcCount + " " + readingCount);
	...
	public void fibonacciDelvedInto() {
		...
		// article says that it can be done in O(logn) using the multiplication
		// matrix how nice!
	...
	public void testLongestIncreasingSubsequence() {
		int result = testLongestIncreasingSubsequence(new int[] { 10, 22, 9,
				33, 21, 50, 41, 60 }, 8);
	...
	private int testLongestCommonSubsequence(String s1, String s2, int x, int y) {
		...
		return Math.max(testLongestCommonSubsequence(s1, s2, x - 1, y),
				testLongestCommonSubsequence(s1, s2, x, y - 1));
	}

	private int testLongestCommonSubsequenceDP(String s1, String s2, int x,
			int y) {
	...
	public void testLongestCommonSubsequence() {
		String s1 = "AGGTAB";
		String s2 = "GXTXAYB";
		int result = testLongestCommonSubsequence(s1, s2, s1.length(),
				s2.length());
		// System.out.println("result of lcs " + result);
		int result2 = testLongestCommonSubsequenceDP(s1, s2, s1.length(),
				s2.length());
